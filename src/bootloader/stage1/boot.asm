...
bdb_media_descriptors_type: db 0F0h                 ; F0 = 3.5" floppy disk
...
ebr_volume_id:              db 00h, 00h, 00h, 01h   ; serial number, value doesn't matter
ebr_volume_label:           db 'OSOS       '        ; 11 bytes, padded with spaces
...
start:
    ...
    push es 
    push word .after
    retf                                      ; far return (pops both the instruction pointer and the code segment)
...
.after:
    ...
    jz .root_dir_after                        ; jumps if test is 0 
...
.load_kernel_loop:
    ...
    add ax, 31                                ; first cluster = (kernel_cluster - 2) * sectors_per_cluster + start_sector
                                              ; start sector = reserved + fats + root dir size = 1 + 18 + 134 = 33 (MT ??)
    mov cl, 1
    mov dl, [ebr_drive_number]
    call disk_read
...
.halt:
    cli                 ; disable interrupts, this prevents the cpu from getting out of the halt state
    hlt
...
.retry:
    ...
    ; test loop condition
    dec di                                  ; decrement di 
    test di, di                             ; test if di equals di (di & di) 
    jnz .retry                              ; if test did not produce a zero flag
